...
Service({
  ...
  "verifySession": Func([UserId], [Opt(UserProfile)], []),
  "viewStory": Func([Nat], [], []),
...
const idlFactory = ({ IDL: IDL2 }) => {
  ...
  return IDL2.Service({
    ...
    "verifySession": IDL2.Func([UserId2], [IDL2.Opt(UserProfile2)], []),
...
function makeStubProfile(userId, username) {
  return {
    id: userId,
    username,
    bio: "",
...
function useCurrentUser() {
  ...
  reactExports.useEffect(() => {
    ...
    const tryEnsure = async (attempts = 0) => {
      try {
    ...
    void (async () => {
      try {
        ...
        if (profile2) {
          ...
          const zarenSession = localStorage.getItem(ZAREN_SESSION_KEY);
          const sessionAge = zarenSession ? Date.now() - Number(zarenSession) : SESSION_VALIDITY_MS + 1;
        ...
        const isNetworkError = msg.includes("IC0508") || msg.includes("canister") || msg.includes("503") || msg.includes("network") || msg.includes("fetch") || msg.includes("unavailable") || msg.includes("timeout");
        const zarenSession = localStorage.getItem(ZAREN_SESSION_KEY);
        const sessionAge = zarenSession ? Date.now() - Number(zarenSession) : SESSION_VALIDITY_MS + 1;
